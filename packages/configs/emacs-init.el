...
(set-face-font 'fixed-pitch "Iosevka Nerd Font") ;; Show same font in codeblocks
...
(use-package evil
  ...
  (evil-define-key nil 'global (kbd "<leader>x") 'consult-flymake)
...
(use-package elm-mode
  :custom (setq elm-mode-hook '(elm-indent-simple-mode))
  :mode "\\.elm\\'")
...
;;   (after-save-hook . (lambda (when (string-match "\\.typ\\'" (buffer-file-name))
;; 			  (typst-compile-buffer))))
...
;;   :config (require 'typst-mode)
;;   (setq typst-allow-compile-other-files t)
